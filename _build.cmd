...
echo Linking...
rgbds\rgblink -v -m %filename%.map -n %filename%.sym -p 0 -d -o %filename%.md25s %filename%.o
...
if %nofix% equ 0 (
	...
	rgbds\rgbfix -v %filename%.md25s -p 0
)

if NOT "%comparison%" == "" if EXIST %comparison% ( fc /B %filename%.md25s %comparison% | more )
goto end
